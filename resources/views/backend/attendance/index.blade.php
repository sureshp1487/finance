<x-app-layout>
    ...
    <style>
        ...
        .mobile-sticky-footer {
            ...
            z-index: 1000; border-top: 1px solid #dee2e6;
        }
        ...
        @media (max-width: 767px) {
            .content-wrapper { padding-bottom: 70px; } /* Space for footer */
        }
    </style>
                ...
                <div class="col-md-6">
                    <div class="d-flex justify-content-between justify-content-md-end gap-2 align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <input type="date" name="date" class="form-control shadow-sm" style="max-width: 160px;" value="{{ $date }}" onchange="window.location.href='{{ route('attendance.index') }}?date='+this.value">
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tr>
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center">
                                                <div class="avatar rounded-circle bg-primary bg-opacity-10 text-primary fw-bold d-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px;">
                                        ...
                                        <td class="text-end pe-4">
                                            <a href="{{ route('attendance.report', $emp->id) }}" class="btn btn-link text-muted btn-sm">
                    ...
                    <div class="card mb-2 shadow-sm border-0 rounded-3">
                        <div class="card-body p-2">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <div class="d-flex align-items-center">
                                    <div class="avatar rounded-circle bg-primary bg-opacity-10 text-primary fw-bold d-flex align-items-center justify-content-center me-2" style="width: 35px; height: 35px; font-size: 0.9rem;">
                                        {{ strtoupper(substr($emp->name, 0, 1)) }}
                                    </div>
                                    <div style="line-height: 1.2;">
                                        <h6 class="mb-0 fw-bold text-dark" style="font-size: 0.95rem;">{{ $emp->name }}</h6>
                                        <small class="text-muted" style="font-size: 0.7rem;">{{ $emp->userProfile->employee_id ?? $emp->id }}</small>
                                    </div>
                                </div>
                            ...
                            <div class="d-flex align-items-center justify-content-between bg-light rounded-2 p-1">
                                ...
                                <div class="btn-group btn-group-sm" role="group">
                                    ...
                                    <label class="btn btn-outline-success fw-bold" style="font-size: 0.75rem; padding: 0.25rem 0.6rem;" for="m_p_{{ $emp->id }}">P</label>

                                    <input type="radio" class="btn-check" name="attendance[{{ $emp->id }}][status]" value="late" id="m_l_{{ $emp->id }}" {{ $status == 'late' ? 'checked' : '' }}>
                                    <label class="btn btn-outline-warning fw-bold text-dark" style="font-size: 0.75rem; padding: 0.25rem 0.6rem;" for="m_l_{{ $emp->id }}">L</label>

                                    <input type="radio" class="btn-check" name="attendance[{{ $emp->id }}][status]" value="absent" id="m_a_{{ $emp->id }}" {{ $status == 'absent' ? 'checked' : '' }}>
                                    <label class="btn btn-outline-danger fw-bold" style="font-size: 0.75rem; padding: 0.25rem 0.6rem;" for="m_a_{{ $emp->id }}">A</label>
                                </div>
                                ...
                                <div class="d-flex align-items-center bg-white border rounded px-1">
                                    ...
                                    <input type="time" class="form-control form-control-sm border-0 shadow-none p-0" style="width: 75px; font-size: 0.85rem;" name="attendance[{{ $emp->id }}][check_in]" value="{{ $check_in }}">
                                </div>
                            </div>
